...
router.get("/springboard", async function (req, res) {
  try {
    const apps = await db.getDB().collection("springboard").find({}).toArray();
    res.render("springboard", { apps: apps });
  } catch (err) {
    console.log(err);
  }
});

router.get("/springboard/:id", async function (req, res) {
  let projectId = req.params.id;
  try {
    projectId = new ObjectId(projectId);
  } catch (err) {
